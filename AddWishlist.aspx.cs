        ...
        try
        {
            if (e.CommandName == "Cart" && e.Item is GridDataItem)
            {
                ...
                if (rd.Read())
                {
                    ...
                    int flg = BusinessTier.AddCart(conn, AddcartID, Convert.ToInt32(ProductID.ToString().Trim()), Convert.ToInt32(Session["CustomerID"].ToString().Trim()), Convert.ToInt32(PricingID.ToString().Trim()), 1, Session["CustomerID"].ToString().Trim(), "U");
                }
                else
                {
                    BusinessTier.DisposeReader(rd);
                    int flg = BusinessTier.AddCart(conn, 1, Convert.ToInt32(ProductID.ToString().Trim()), Convert.ToInt32(Session["CustomerID"].ToString().Trim()), Convert.ToInt32(PricingID.ToString().Trim()), 1, Session["CustomerID"].ToString().Trim(), "N");
                }
                BusinessTier.DisposeConnection(conn);
                Response.Redirect("AddCart.aspx?Param=" + Session["CustomerID"].ToString(), false);
            }
