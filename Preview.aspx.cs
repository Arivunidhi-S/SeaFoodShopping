    ...
    protected void btnAddCart_OnClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (rd.Read())
            {
                ...
                {
                    int flg = BusinessTier.AddCart(conn, AddcartID, Convert.ToInt32(@Param.ToString().Trim()), Convert.ToInt32(Session["CustomerID"].ToString().Trim()), pricingid, Convert.ToDecimal(hdqnty.Value), Session["CustomerID"].ToString().Trim(), "U");
                }
                else
                {
                    int flg = BusinessTier.AddCart(conn, AddcartID, Convert.ToInt32(@Param.ToString().Trim()), Convert.ToInt32(Session["CustomerID"].ToString().Trim()), pricingid, qty, Session["CustomerID"].ToString().Trim(), "U");
                }
            }
            else
            {
                BusinessTier.DisposeReader(rd);
                int flg = BusinessTier.AddCart(conn, 1, Convert.ToInt32(@Param.ToString().Trim()), Convert.ToInt32(Session["CustomerID"].ToString().Trim()), pricingid, Convert.ToDecimal(cboQuantity.Text.ToString().Trim()), Session["CustomerID"].ToString().Trim(), "N");
            }
